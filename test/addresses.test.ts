...
describe("good addresses pass", function () {
    ...
    it("short TLD", function () {
        _check("admin@foo.x", "admin", undefined, "foo.x", undefined);
    });
...
describe("bad addresses fail", function () {
    ...
    it("out of range v4 octet [300.0.0.1]", function () {
        assert.throws(function () {
            parse("user@[300.0.0.1]");
        });
    });
    it("too many v4 octets [127.0.0.0.1]", function () {
        assert.throws(function () {
            parse("user@[127.0.0.0.1]");
        });
    });
    it("leading zero v4 octet [127.00.0.1]", function () {
        assert.throws(function () {
            parse("user@[127.00.0.1]");
        });
    });
    it("not enough v4 octets [127.0.1]", function () {
        assert.throws(function () {
            parse("user@[127.0.1]");
        });
    });
    ...
    it("two elided zero ranges in v6 address [IPv6:68::1c:a2::e5]", function () {
        assert.throws(function () {
            parse("user@[IPv6:68::1c:a2::e5]");
        });
    });
    it("invalid char in domain user@example.com#", function () {
        assert.throws(function () {
            parse("user@example.com#");
        });
    });
    it("trailing dot on domain user@example.com.", function () {
        assert.throws(function () {
            parse("user@example.com.");
        });
    });
    it("space in local-part foo bar@example.com", function () {
        assert.throws(function () {
            parse("foo bar@example.com");
        });
    });
    it("double dot in local-part john..doe@example.com", function () {
        assert.throws(function () {
    ...
    it("i_like_underscore@but_its_not_allowed_in_domain_part.example.com", function () {
        assert.throws(function () {
            parse("i_like_underscore@but_its_not_allowed_in_domain_part.example.com");
        });
    });
